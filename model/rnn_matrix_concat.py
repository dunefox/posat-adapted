...
class RelationModel(object):
    ...
    def __init__(self, opt, emb_matrix=None):
        ...
        # eigener Code
        self.reg_matrix = torch.cat((torch.eye(42), torch.zeros(42, 200)), dim=1).cuda()
    ...
    def update(self, batch):
        ...
        #  loss = torch.add(loss, torch.tensor(torch.mul(torch.tensor(self.weight), squared_sum))) # , requires_grad=True)
    ...
    def predict(self, batch, unsort=True):
        ...
        # forward
        self.model.eval()
        logits, _, weights = self.model(inputs)
        loss = self.criterion(logits, labels)
        ...
        if unsort:
            _, predictions, probs = [list(t) for t in zip(*sorted(zip(orig_idx,\
                    predictions, probs)))]
        return predictions, probs, loss.data.item(), weights
...
class PositionAwareRNN(nn.Module):
    ...
    def __init__(self, opt, emb_matrix=None):
        ...
        # eigener Code
        # Neue LinearLayer mit Eingaben aus Hidden und urspr. Ausgabeschicht
        # Dimensionen 42 x 242
        self.adapt = nn.Linear(opt['num_class'] + opt['hidden_dim'], opt['num_class'])
